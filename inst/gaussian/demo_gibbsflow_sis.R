...
compute_gibbsflow <- function(stepsize, current_time, xparticles, logdensity){
  
}

# Gibbsflow SIS settings
nparticles <- 2^10
nsteps <- 40
...
# run SMC
tic("Gibbsflow SIS runtime")
smc <- run_gibbsflow_sis(model$prior, model$likelihood, nparticles, lambda, timegrid)
toc()
