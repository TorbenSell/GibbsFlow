...
double prior_rootisum = arma::sum(log(prior_rooti.diag()));
...
double gaussian_log_normconst(double lambda){
  ...
  double output = - lambda * (-(static_cast<double>(dimension)/2.0) * log2pi + like_rootisum ) - 
...
Rcpp::List gaussian_gibbsflow(double stepsize, double lambda, double derivative_lambda, 
  ...
  for (int n = 0; n < N; n++){
    ...
    for (int i = 0; i < dimension; i++){
      ...
      conditional_cdf = lowerintegral_target / (lowerintegral_target + upperintegral_target);
